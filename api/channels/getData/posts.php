...
try {
    ...
    $stmt = $pdo->prepare('SELECT posts FROM channels_posts WHERE channel_id = :channel_id ORDER BY created_at DESC LIMIT 1');
    $stmt->bindValue(':channel_id', $channel_id, PDO::PARAM_INT);
    $stmt->execute();
    $result = $stmt->fetch(PDO::FETCH_ASSOC);

    if (empty($result) || !isset($result['posts'])) {
    ...
    // Декодируем JSON поле posts
    $postsData = json_decode($result['posts'], true);
    ...
    if (json_last_error() !== JSON_ERROR_NONE || !is_array($postsData)) {
